...
export class CourseCard implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // When @Input course changes, the map above will automatically pick up the new this.course.id 
    // for subsequent emits, but ideally we'd pass the id in the pipe or just let it read this.course.id.
    // However, the constructor runs before inputs are set. So we should re-initialize or do it in template.
    // Let's do it in a getter or just use the template. But we'll leave this as-is since the template 
    // will use async pipe and we can just pass the id in the method.
  }
