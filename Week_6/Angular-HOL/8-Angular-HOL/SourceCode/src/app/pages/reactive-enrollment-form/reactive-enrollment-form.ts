...
export class ReactiveEnrollmentForm implements OnInit, CanComponentDeactivate {
  ...
  onSubmit() {
    ...
    if (this.enrollForm.valid) {
      ...
      // Step 81: Wire the createCourse HTTP call to the form submission.
      // Mapping the student form to a new course just for demonstration.
      const newCourse = {
        name: formValue.studentName + ' Request',
      ...
      this.courseService.createCourse(newCourse).subscribe({
        next: (created: any) => {
          console.log('Course created via form:', created);
          this.submitted = true;
          this.enrollForm.markAsPristine(); // clear dirty state for the guard
        },
